...
**Plans**: 5 plans (original) + 1 cherry-picked gap fix (05-07); alt-conf gap closure (05-06/05-08) attempted + reverted, deferred to post-v1
...
- [-] 05-06-PLAN.md — Alt-conf research spike (gap closure) — DEFERRED to post-v1 (work preserved in `backup/05-08-attempts` branch; spike proved mechanism viable in isolation but multi-rep integration revealed cascading GUI-only failures)
- [-] 05-08-PLAN.md — Alt-conf cartoon/ribbon implementation (gap closure) — DEFERRED to post-v1 (4 failed GUI fix cycles, each passed headless verification but failed in GUI; root cause: headless cannot reproduce GUI auto_zoom=1 / multi-state display / retroactive coord corruption)

**Phase 5 status: PARTIAL** — Line/stick/sphere + terminal-extension cartoon hiders work (1znf mixed-rep confirmed). Remaining v1 limitations (documented, functional not cosmetic-blocking):
- Cartoon terminal-extension renders disconnected on 1ubq (cosmetic — hiders clickable, game playable)
- Ribbon rep unsupported (cartoon hardcoded in show call — simple fix possible)
- Alt-conf segment replication (the roadmap's "replicate a segment as alternate position") deferred to post-v1: researched + spike-verified in isolation, but integration with multi-rep + GUI runtime too complex for v1
